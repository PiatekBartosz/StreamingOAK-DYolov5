...
class RobotDeltaClient(threading.Thread):
    ...
    def start_sorting(self, stdscr):
        stdscr.clear()
        stdscr.addstr(0, 0, "Press any key to start sorting...")
        stdscr.refresh()
        ...
        while True:
            ...
            # Read the value from the shared queue
            value = self.shared_queue.get_queue()
            stdscr.addstr(1, 0, f"Shared Value: {value}")
            ...
            key = stdscr.getch()
            if key == ord('q'):
                self.join()  # Wait for the RobotDeltaClient thread to finish
                break

    def run(self):
        with curses.wrapper(self.start_sorting):
            pass
